...
export const google = async (req, res, next) => {
  ...
  try {
    // first we check if user exist or not,if exist we sign in if not we create user

    const user = await User.findOne({ email: req.body.email })

    // if user exist we register 
    if (user) {
    ...
    // otherwise we wanna create a new user
    else {
      const generatePassword = Math.random().toString(36).toString(-8)
